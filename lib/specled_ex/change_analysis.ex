defmodule SpecLedEx.ChangeAnalysis do
  ...
  def analyze(index, root, opts \\ []) do
    ...
    %{
      ...
      uncovered_policy_files:
        Enum.filter(policy_files, &(Map.get(impacted_by_file, &1, []) == [])),
  ...
  def detect_base_ref(_root, explicit_base) when is_binary(explicit_base), do: explicit_base

  def detect_base_ref(root, nil) do
