...
- `mix spec.cover.test`
  ...
  - `--per-test` opts into a serialized per-test attribution lane that is exact within disclosed chained windows when the boundary hook is wired (`--allow-async` degrades instead of failing on `async: true` contamination)
