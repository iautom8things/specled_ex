...
- id: specled.tag_scanning.describe_block_recursion
  ...
  priority: must
  stability: evolving
- id: specled.tag_scanning.for_comprehension_recursion
  statement: >-
    The tag scanner shall walk into `for` comprehension bodies so that
    `test/2` definitions generated by a comprehension pick up the `@tag spec`
    ids declared inside that body, inherit the enclosing module's
    `@moduletag spec` ids and any enclosing `@describetag spec` ids, and have
    their non-literal `@tag spec` values reported as dynamic entries. The body
    is walked once regardless of how many iterations it unrolls, so the tag map
    records the presence of a tag carrier rather than one entry per iteration.
  priority: must
  stability: evolving
```
...
- id: specled.tag_scanning.scenario.for_comprehension_generated_test
  given:
    - a test file whose module body wraps an `@tag spec` annotation and a `test/2` definition in a `for` comprehension
  when:
    - SpecLedEx.TagScanner.scan_file/1 runs on that file
  then:
    - the returned tag list contains the annotated requirement id linked to that file
  covers:
